...
export default function CartAndPrice({ blockVar, handleAddToWishlist, loading, onAdding, stock, isWishlistAdding }) {
    return (
        <div className="m-4 grid gap-4 sm:grid-cols-2">

            <button disabled={stock === 0 || loading || !blockVar} onClick={() => onAdding()}
        ...
        hover:bg-gray-900 active:scale-95 transition-all duration-200 shadow-md"
            ...
            <button onClick={handleAddToWishlist} disabled={isWishlistAdding || blockVar}
                className="flex items-center justify-center gap-2 
